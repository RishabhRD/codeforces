...
auto solve(ll _t) {
  ...
  auto const ds = read_vec<ll>(n);
  auto const as = read_vec<ll>(n);

  std::vector dp(2, std::vector(n + 1, std::vector(n + 1, -1ll)));
  for (ll num_rem = 0; num_rem <= k; ++num_rem) {
    for (ll cur = n; cur >= 1; --cur) {
      for (ll prev = n - 1; prev >= 0; --prev) {
        if (cur == n) {
          dp[1][cur][prev] = (l - ds[prev]) * as[prev];
        } else {
          ll res = (ds[cur] - ds[prev]) * as[prev] + dp[1][cur + 1][cur];
          if (num_rem > 0) {
            res = std::min(res, dp[0][cur + 1][prev]);
          }
          dp[1][cur][prev] = res;
        }
      }
