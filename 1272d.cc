...
auto val_of(ll prev, ll cur) {
  if (prev >= cur)
    return 1;
  return 0;
}

ll dfs(ll i, ll picked, std::vector<ll> const &nums,
  ...
  if (!picked) {
    if (i + 2 < n && nums[i + 2] > nums[i]) {
      ans = std::max(ans, 1 + dfs(i + 2, true, nums, dp));
    }
    if (i + 1 < n && nums[i + 1] > nums[i]) {
      ans = std::max(ans, 1 + dfs(i + 1, false, nums, dp));
    }
  } else {
    if (nums[i + 1] <= nums[i]) {
      ans = 1;
    } else {
      ans = 1 + dfs(i + 1, true, nums, dp);
    }
  }
  return dp[i][picked] = ans;
}
...
auto solve(ll _t) {
  ...
  ll max = 0;
  for (ll i = 0; i < n; ++i) {
    max = std::max(max, dfs(i, false, nums, dp));
  }
  return max;
}
