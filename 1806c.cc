...
auto solve(ll _t) {
  ...
  auto const nums = make_sorted(read_vec<ll>(2 * n));

  if (n == 1) {
    auto const mid = (nums[0] + nums[1]) / 2;
    return std::abs(nums[0] - mid) + std::abs(nums[1] - mid);
  } else {
    auto ans = nums                            //
               | vw::transform(lift(std::abs)) //
               | rd::sum;
    if (n == 2) {
      ans = nums | vw::transform(rd::bind_back(std::minus<>{}, 2ll)) //
            | vw::transform(lift(std::abs))                          //
            | rd::sum                                                //
            | rd::pipeable{rd::bind_back(lift(std::min<ll>), ans)};  //
    }
    if (n % 2 == 0) {
      ans = nums | vw::take(2 * n - 1) |
            vw::transform(rd::bind_back(std::minus<>{}, -1ll)) //
            | vw::transform(lift(std::abs))                    //
            | rd::sum                                          //
            | rd::pipeable(
                  rd::bind_back(std::plus<>{}, std::abs(nums.back() - n))) //
            | rd::pipeable{rd::bind_back(lift(std::min<ll>), ans)};        //
    }
    return ans;
  }
