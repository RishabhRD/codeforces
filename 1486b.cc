...
auto solve(ll _t) {
  ...
  auto dist = [](std::vector<ll> const &nums, ll i) {
    ll sum = 0;
    for (auto n : nums) {
      sum += std::abs(n - i);
    }
  ...
  auto dist_x = std::bind_front(dist, xs);
  auto dist_y = std::bind_front(dist, ys);

  ll xl = *rng::partition_point(
      vw::iota(-1ll, mod), [&](auto n) { return dist_x(n) > dist_x(n + 1); });
  ll xh = *rng::partition_point(
      vw::iota(-1ll, mod), [&](auto n) { return dist_x(n) <= dist_x(n - 1); });
  ll yl = *rng::partition_point(
      vw::iota(-1ll, mod), [&](auto n) { return dist_y(n) > dist_y(n + 1); });
  ll yh = *rng::partition_point(
      vw::iota(-1ll, mod), [&](auto n) { return dist_y(n) <= dist_y(n - 1); });

  std::cout << (xh - xl) * (yh - yl) << endl;
}
