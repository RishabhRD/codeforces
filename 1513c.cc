...
ll dfs(ll i, ll m, std::vector<std::vector<ll>> &dp) {
  if (m == 0)
    return 1;
  ...
  ll sum = 0;
  if (i < 9) {
    sum = (sum + dfs(i + 1, m - 1, dp)) % mod;
  } else {
    sum = (sum + dfs(1, m - 1, dp)) % mod;
    sum = (sum + dfs(0, m - 1, dp)) % mod;
  }

  return dp[i][m] = sum;
}

auto solve(ll _t, std::vector<std::vector<ll>> &dp) {
...
int main() {
  ...
  std::cin.tie(0);
  std::vector dp(10, std::vector(2e5 + 1, -1ll));
  auto t = read<ll>();
