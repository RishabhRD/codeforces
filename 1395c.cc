...
bool is_fit(ll req, ll cur) { return req == (req | cur); }

auto solve(ll _t) {
  auto const n = read<ll>();
  auto const m = read<ll>();
  auto const a = read_vec<ll>(n);
  auto const b = read_vec<ll>(m);

  for (ll req_val = 0; req_val < 512; ++req_val) {
    if (std::all_of(std::begin(a), std::end(a), [&](auto na) {
          return std::any_of(std::begin(b), std::end(b),
                             [&](auto nb) { return is_fit(req_val, na & nb); });
        })) {
      return req_val;
    }
  }
