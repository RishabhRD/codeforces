...
std::vector<ll> const factorials{
    1,     2,      6,       24,       120,       720,        5040,
    40320, 362880, 3628800, 39916800, 479001600, 6227020800, 87178291200,
};

auto solve(ll _t) {
  auto const sz = factorials.size();
  auto const n = read<ll>();

  ll min = limits<ll>::max();
  for (ll i = 0; i < (1 << sz); ++i) {
    std::bitset<32> bs(i);
    ll sum = 0;
    for (ll i = 0; i < 32; ++i) {
    ...
    if (rem < 0)
      ...
      std::bitset<64> new_bs(rem);
      if ((bs[0] == 1 && new_bs[0] == 1) || (bs[1] == 1 && new_bs[1] == 1)) {
        continue;
      }
      min = std::min<ll>(min, bs.count() + new_bs.count());
    }
  }
  std::cout << min << endl;
}
