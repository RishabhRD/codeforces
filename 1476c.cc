...
auto solve(ll _t) {
  ...
  prefix[1] = std::abs(b[1] - a[1]) + 2;
  for (ll i = 2; i < n; ++i) {
    if (a[i] == b[i]) {
      prefix[i] = 2;
    } else {
      ll const min = std::min(a[i], b[i]);
      ll const max = std::max(a[i], b[i]);

      prefix[i] =
          std::max(prefix[i - 1] + min - 1 + c[i - 1] - max + 2, max - min + 2);
    }
  }
  ...
  ll max = 0;
  for (ll i = 1; i < n; ++i) {
    max = std::max(max, prefix[i] + c[i] - 1);
  }
