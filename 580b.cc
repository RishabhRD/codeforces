...
public:
  ...
  template <typename... Xs>
    requires std::is_invocable<F, Xs...>::value
...
auto make_sorted_with =
    rd::pipeable([]<typename Container, typename Comparator>(Container vec,
                                                             Comparator &&cmp) {
      rng::sort(vec, cmp);
      return vec;
    });

auto make_sorted = make_sorted_with(std::less<>{});
...
namespace rd {
auto compare_on =
    []<typename F, typename Cmp = std::less<>>(F f, Cmp cmp = std::less<>{}) {
  return [f = std::move(f), cmp = std::move(cmp)](
             auto const &a, auto const &b) { return cmp(f(a), f(b)); };
};
} // namespace rd
...
auto solve(ll _t) {
  ...
  auto const friends = vw::iota(0, n) //
                       | vw::transform([](auto) {
                       ...
                       | rd::to<std::vector>() //
                       | make_sorted_with(rd::compare_on(lift(std::get<0>)));
  auto prefix =
      vw::transform(friends, lift(std::get<1>)) | rd::to<std::vector>();
  for (ll i = 1; i < n; ++i) {
    prefix[i] += prefix[i - 1];
  }

  auto sum_of = [&](ll i, ll j) {
    if (i == 0)
      return prefix[j];
  ...
  ll max = 0;
  for (ll i = 0; i < n; ++i) {
    auto is_good_friend = [&](auto j) {};
    auto const j = *rng::partition_point(vw::iota(i + 1, n), [&](auto j) {
      return friends[j].first - friends[i].first < d;
    }) - 1;

    max = std::max(max, sum_of(i, j));
  }
