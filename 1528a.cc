...
ll dfs(ll i, ll parent, ll is_left, std::vector<std::vector<ll>> const &graph,
       std::vector<std::pair<ll, ll>> const &ranges,
       std::vector<std::vector<ll>> &dp) {
  if (dp[i][is_left] != -1)
    return dp[i][is_left];

  ll parent_val = is_left ? ranges[parent].first : ranges[parent].second;
  ll left_sum = std::abs(ranges[i].first - parent_val);
  ll right_sum = std::abs(ranges[i].second - parent_val);

  for (auto n : graph[i]) {
    if (n == parent)
      continue;

    left_sum += dfs(n, i, true, graph, ranges, dp);
    right_sum += dfs(n, i, false, graph, ranges, dp);
  }
  return dp[i][is_left] = std::max(left_sum, right_sum);
}
...
auto solve(ll _t) {
  ...
  std::vector<std::pair<ll, ll>> ranges(n + 1);
  for (ll i = 0; i < n; ++i) {
  ...
  std::vector dp(n + 1, std::vector(2, -1ll));

  ll left_sum = 0;
  for (ll n : graph[1]) {
    left_sum += dfs(n, 1, true, graph, ranges, dp);
  }

  ll right_sum = 0;
  for (ll n : graph[1]) {
    right_sum += dfs(n, 1, false, graph, ranges, dp);
  }

  std::cout << std::max(left_sum, right_sum) << endl;
}
