...
std::vector dp(1e3 + 1, std::vector(1e3 + 1, -1ll));

ll num_required(ll a, ll b) {
  if (a == b)
    return dp[a][b] = 0;
  if (dp[a][b] != -1ll)
    return dp[a][b];
  ...
  for (ll i = 1; i <= a; ++i) {
    if (a + a / i <= b)
      min = std::min(min, 1 + num_required(a + a / i, b));
  }
  return dp[a][b] = min;
}

ll dfs(ll i, ll k, std::vector<ll> const &weights,
       std::vector<ll> const &reward, std::vector<std::vector<ll>> &dp) {
  if (i >= weights.size())
    return 0;
  ...
  ll max = 0;
  max = std::max(max, dfs(i + 1, k, weights, reward, dp));
  if (weights[i] <= k)
    max = std::max(max,
                   reward[i] + dfs(i + 1, k - weights[i], weights, reward, dp));
  return dp[i][k] = max;
}

auto solve(ll _t) {
  auto const n = read<ll>();
  auto const k = std::min(read<ll>(), ll(12000));
  auto b = read_vec<ll>(n);
  auto const c = read_vec<ll>(n);
  for (auto &w : b) {
    w = num_required(1, w);
    slog << w << ' ';
  }
