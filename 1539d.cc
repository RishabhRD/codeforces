...
auto solve(ll _t) {
  ...
  std::vector<std::pair<ll, ll>> products;
  for (ll i = 0; i < n; ++i) {
  ...
  ll total_bought = 0;
  while (low <= high) {
    if (total_bought >= products[low].second) {
      total_bought += products[low].first;
      sum += products[low].first;
      products[low].first = 0;
      ++low;
    } else {
      ll const extra_req = products[low].second - total_bought;

      if (extra_req >= products[high].first) {
        sum += 2 * products[high].first;
        total_bought += products[high].first;
        products[high].first = 0;
        --high;
      } else {
        sum += 2 * extra_req;
        total_bought += extra_req;
        products[high].first -= extra_req;
      }
    }
  }
