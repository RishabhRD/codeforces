...
auto solve(ll _t) {
  ...
  auto const matrix = read_vec<std::string>(n);

  std::vector<std::vector<ll>> cost(n, std::vector<ll>(n, 0ll));
  std::vector<ll> col_sum(n);
  for (ll j = 0; j < n; ++j) {
    ll sum = 0;
    for (ll i = 0; i < n; ++i) {
  ...
  for (ll i = 0; i < n; ++i) {
    for (ll j = 0; j < n; ++j) {
      cost[i][j] = matrix[i][j] == '0' ? (col_sum[j] + 1) : (col_sum[j] - 1);
    }
  }
