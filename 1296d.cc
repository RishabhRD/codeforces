...
auto solve(ll _t) {
  ...
  auto get_num_k_required = [a, b](auto h) -> ll {
    h = h % (a + b);
    if (h == 0) {
      return std::ceil(b / static_cast<double>(a));
    }
    return std::ceil(h / static_cast<double>(a)) - 1;
  };

  auto const hs = make_sorted(read_vec<ll>(n), [&](auto a, auto b) {
    return get_num_k_required(a) < get_num_k_required(b);
  });

  ll num = 0;
  for (auto h : hs) {
    auto const req = get_num_k_required(h);
    if (req <= k) {
      k -= req;
      ++num;
    }
