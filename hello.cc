...
auto sum = rd::pipeable([](auto a, auto b) { return a + b; });

auto id = rd::pipeable([](auto n) { return n - 1; });
...
auto solve(ll _t) {
  ...
  // 7
  //
  return read_vec<ll>(30)                  //
         | make_sorted                     //
         | rd::group_by(std::equal_to<>{}) //
         | vw::transform(rng::distance)    //
         | rd::sum;                        //
}
