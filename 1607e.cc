...
auto solve(std::string_view str, char a, char b, ll n) {
  auto idx_for = [&](ll len) {
    ll max = 0;
    ll min = 0;
  ...
  auto const max_valid =
      binary_search(0, str.size() + 1,
                    [&](auto len) { return idx_for(len) != -1; }) -
      1;

  return idx_for(max_valid);
}
...
auto solve(ll _t) {
  ...
  auto hor = vw::filter(str, [](auto c) { return c == 'L' || c == 'R'; }) |
             rd::to<std::string>();

  auto ver = vw::filter(str, [](auto c) { return c == 'D' || c == 'U'; }) |
             rd::to<std::string>();

  auto const c = solve(hor, 'L', 'R', m);
  auto const r = solve(ver, 'U', 'D', n);

  std::cout << r + 1 << ' ' << c + 1 << endl;
}
