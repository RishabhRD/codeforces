...
auto solve(ll _t) {
  ...
  auto const as = read_vec<ll>(n);
  auto const bs = read_vec<ll>(n);

  std::vector<std::tuple<ll, ll, ll>> strengths;
  for (ll i = 0; i < n; ++i) {
    strengths.push_back({as[i], bs[i], i});
  }
  ...
  for (ll i = 0; i < n - 1; ++i) {
    graph[std::get<2>(strengths[i + 1])].push_back(std::get<2>(strengths[i]));
  }
  ...
  auto can_win = [&](ll i) {
    ...
    dfs(std::get<2>(strengths[i]), graph, visited);
    return rng::all_of(visited, [](auto e) { return e == true; });
  };

  std::string res(n, '0');
  auto const j = binary_search(0, n, std::not_fn(can_win));
  for (ll i = j; i < n; ++i) {
    res[std::get<2>(strengths[i])] = '1';
  }
