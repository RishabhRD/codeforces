...
auto solve(ll _t) {
  ...
  auto const left = make_sorted(read_vec<ll>(l));
  auto const right = make_sorted(read_vec<ll>(r));

  std::vector<ll> rem_left;
  std::vector<ll> rem_right;

  {
    ...
    while (i < l && j < r) {
      if (left[i] == right[j]) {
        ...
        rem_left.push_back(left[i]);
        ++i;
      } else {
        rem_right.push_back(right[j]);
        ++j;
      }
    }
    while (i < l) {
  ...
  auto make_freq = rd::group_by(std::equal_to<>{}) |
                   rd::pipeable(vw::transform([](auto &&rng) {
                     return std::pair{*rng.begin(), rng::distance(rng)};
                   })) |
                   rd::pipeable{lift(rd::to<std::vector>)};
  auto count_ele =
      rd::pipeable{vw::transform([](auto e) { return e.second; })} | rd::sum;
  auto left_freq = make_freq(rem_left);
  auto right_freq = make_freq(rem_right);
  auto num_left = count_ele(left_freq);
  auto num_right = count_ele(right_freq);

  if (num_left < num_right) {
    std::swap(left_freq, right_freq);
    std::swap(num_left, num_right);
  }

  ll sum = 0;
  for (auto [e, freq] : left_freq) {
    auto diff = (num_left - num_right) / 2;
    auto const can_do = std::min(freq / 2, diff);
    num_left -= can_do * 2;
    sum += can_do;
  }
