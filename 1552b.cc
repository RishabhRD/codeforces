...
auto solve(ll _t) -> std::optional<ll> {
  ...
  for (ll i = 0; i < n; ++i) {
    results.push_back(read_vec<ll>(5));
  }

  std::vector<ll> persons(n);
  std::iota(std::begin(persons), std::end(persons), 0);
  ...
  auto const min = *std::min_element(std::begin(persons), std::end(persons),
                                     [&](auto a, auto b) { return cmp(a, b); });
  for (ll i = 0; i < n; ++i) {
    if (i == min)
      continue;
    if (!cmp(min, i))
...
int main() {
  ...
  for (ll i = 0; i < t; ++i) {
    ...
    auto const val = solve(i);
    if (val.has_value()) {
      std::cout << val.value() << endl;
    } else {
      std::cout << -1 << endl;
    }
  }
