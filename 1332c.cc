...
auto solve(ll _t) {
  ...
  std::vector freq(k, std::vector(26, 0ll));
  for (ll i = 0; i < n; ++i) {
    ++freq[i % k][str[i] - 'a'];
  }
  std::vector c_freq((k + 1) / 2, std::vector(26, 0ll));
  for (ll i = 0; i < k / 2; ++i) {
    auto const j = k - i - 1;
    for (ll k = 0; k < 26; ++k) {
      c_freq[i][k] = freq[i][k] + freq[j][k];
    }
  }
  if (k % 2) {
    ll const i = k / 2;
  ...
  for (ll i = 0; i < (k + 1) / 2; ++i) {
    auto const max = *rng::max_element(c_freq[i]);
    auto const total = rd::sum(c_freq[i]);
    sum += total - max;
  }
