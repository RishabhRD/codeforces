...
template <typename T> auto waste_read(ll n) {
  for (ll i = 0; i < n; ++i)
    read<T>();
}
...
auto solve(ll _t) {
  ...
  auto const last_unsorted_idx = [&]() -> std::optional<ll> {
    for (ll i = n - 1; i >= 0; --i) {
      if (nums[i] != i + 1) {
        return i + 1;
      }
  ...
  if (!last_unsorted_idx.has_value()) {
    for (ll i = 0; i < m; ++i) {
      read<ll>();
      read<double>();
    }
  ...
  for (ll i = 0; i < m; ++i) {
    ...
    auto const b = read<double>();
    if (a < last_unsorted_idx.value()) {
      continue;
    }
    mul *= (1 - b);
  }
...
int main() {
  ...
  for (ll i = 0; i < t; ++i) {
    ...
    std::cout << std::setprecision(6) << solve(i) << endl;
  }
}
