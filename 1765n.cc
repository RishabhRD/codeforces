...
void dfs(ll i, ll k, bool to_include_zero, auto &&get_sum, ll n,
         std::string_view str, std::string &res) {
  if (i + k == n) {
    return;
  }
  if (k == 0) {
    std::copy(std::begin(str) + i, std::end(str), std::back_inserter(res));
    return;
  }
  auto const j = [&] {
    for (ll j = 0; j < 10; ++j) {
      if (j == 0 && to_include_zero == false)
        continue;
      auto const idx = *rng::partition_point(
          vw::iota(i, n), [&](auto k) { return get_sum(j, i, k) < 1; });
      if (idx <= i + k)
        return idx;
    }
    return -1ll;
  ...
  res.push_back(str[j]);
  dfs(j + 1, k - (j - i), true, get_sum, n, str, res);
}
...
auto solve(ll _t) {
  ...
  std::vector prefix(10, std::vector(n, 0ll));
  for (ll j = 0; j < n; ++j) {
    auto const c = str[j] - '0';
    prefix[c][j] = 1;
  }
  ...
  auto get_sum = [&](auto n, auto i, auto j) {
    if (i == 0) {
      return prefix[n][j];
    } else {
      return prefix[n][j] - prefix[n][i - 1];
    }
  };

  std::string res;
  dfs(0, k, false, get_sum, n, str, res);
  std::cout << res << endl;
}
