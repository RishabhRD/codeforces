...
auto solve(ll _t) {
  ...
  if ((m + n) % 2 == 0)
    return false;
  std::vector max1(m, std::vector(n, -inf));
  std::vector min1(m, std::vector(n, inf));
  ...
  for (ll i = 0; i < m; ++i) {
    for (ll j = 0; j < n; ++j) {
      if (i + 1 < m) {
        max1[i + 1][j] =
            std::max(max1[i + 1][j], max1[i][j] + matrix[i + 1][j]);
        min1[i + 1][j] =
            std::min(min1[i + 1][j], min1[i][j] + matrix[i + 1][j]);
      }
      if (j + 1 < n) {
        max1[i][j + 1] =
            std::max(max1[i][j + 1], max1[i][j] + matrix[i][j + 1]);
        min1[i][j + 1] =
            std::min(min1[i][j + 1], min1[i][j] + matrix[i][j + 1]);
      }
    }
  }

  // slog << _t << ' ' << min1[m - 1][n - 1] << ' ' << max1[m - 1][n - 1] <<
  // endl;

  return max1[m - 1][n - 1] >= 0 && min1[m - 1][n - 1] <= 0;
}
