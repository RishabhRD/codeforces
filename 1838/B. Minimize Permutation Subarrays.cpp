...
auto solve(ll _t) {
  ...
  auto const one_idx = rng::find(nums, 1) - rng::begin(nums) + 1;
  auto const two_idx = rng::find(nums, 2) - rng::begin(nums) + 1;
  auto const n_idx = rng::find(nums, n) - rng::begin(nums) + 1;

  if ((one_idx < n_idx && n_idx < two_idx) ||
      (one_idx > n_idx && n_idx > two_idx)) {
    std::cout << one_idx << ' ' << one_idx << endl;
  } else if ((one_idx < two_idx && two_idx < n_idx) ||
             (one_idx > two_idx && two_idx > n_idx)) {
    std::cout << two_idx << ' ' << n_idx << endl;
  } else {
    std::cout << one_idx << ' ' << n_idx << endl;
  }
}
