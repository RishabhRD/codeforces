...
auto solve(ll _t) {
  ...
  auto sum = vw::transform(nums, math::divide_by(k)) | rd::sum;
  auto const mods = vw::transform(nums, math::mod_by(k)) //
                    | rd::to<std::vector>()              //
                    | make_sorted;
  std::vector<ll> freq(k);
  {
    auto rng_ad = rd::group_by(std::equal_to<>{}) //
                  | rd::pipeable{vw::transform([](auto rng) {
                      return std::pair{*rng.begin(), ll(rng::distance(rng))};
                    })};
    auto rng = mods | rng_ad;
    for (auto [mod, mod_freq] : rng) {
      freq[mod] = mod_freq;
    }
  }
  ll low = 1;
