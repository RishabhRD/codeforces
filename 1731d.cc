...
auto make_prefix_matrix(std::vector<std::vector<ll>> const &matrix) {
  auto res = matrix;
  res[0][0] = matrix[0][0];
...
auto make_range_2d_sum(std::vector<std::vector<ll>> const &prefix_matrix) {
  return [&](ll x1, ll y1, ll x2, ll y2) {
    ll res = prefix_matrix[x2][y2];
    if (x1 > 0) {
      res -= prefix_matrix[x1 - 1][y2];
    }
    if (y1 > 0) {
      res -= prefix_matrix[x2][y1 - 1];
    }

    if (x1 > 0 && y1 > 0) {
      res += prefix_matrix[x1 - 1][y1 - 1];
    }
...
auto solve(ll _t) {
  ...
  std::vector matrix(m, std::vector(n, 0ll));
  for (ll i = 0; i < m; ++i) {
    for (ll j = 0; j < n; ++j)
      std::cin >> matrix[i][j];
  }
  ...
  auto can_make = [&](ll l) {
    std::vector cell_entry(m, std::vector(n, 0ll));
    for (ll i = 0; i < m; ++i) {
      for (ll j = 0; j < n; ++j) {
        if (matrix[i][j] >= l)
          cell_entry[i][j] = 1;
    ...
    auto prefix_matrix = make_prefix_matrix(cell_entry);
    auto sum_till = make_range_2d_sum(prefix_matrix);
    for (ll i = 0; i < m; ++i) {
      for (ll j = 0; j < n; ++j) {
        if (i + l > m || j + l > n)
          continue;
        if (l == 4 && i == 1 && j == 2)
          ...
          log_enabled = false;
        ll const cur_sum = sum_till(i, j, i + l - 1, j + l - 1);
        if (cur_sum == l * l) {
          return true;
        }
      }
  ...
  std::cout << binary_search(0, std::max(m, n) + 1, can_make) - 1 << endl;
}
