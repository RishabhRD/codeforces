...
auto solve(ll _t) {
  ...
  dsu dsu(n);
  for (ll i = 0; i < n; ++i) {
    dsu.combine(i, nums[i]);
  }

  std::map<ll, std::vector<ll>> sets;
  for (ll i = 0; i < n; ++i) {
  ...
  ll const open_contri = num_open == 0 ? 0 : 1;
  ll const closed_contri = num_closed;

  std::cout << open_contri + closed_contri << ' ' << sets.size() << endl;
}
