...
auto solve(ll _t) {
  ...
  auto const prime_factors = make_prime_factors(q);

  // for (auto [k, v] : prime_factors) {
  //   slog << k << ' ' << v << endl;
  // }

  ll ans = limits<ll>::min();
  for (auto [f, fcnt] : prime_factors) {
    ll n = p;
    ll cnt = 0;
    while (n % f == 0) {
      n /= f;
