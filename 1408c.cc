...
ll binary_search(ll low, ll high, Predicate const &predicate) {
  while (low < high) {
...
auto solve(ll _t) {
  ...
  auto coord_after = [&](double t) {
    // slog << std::fixed << t << endl;
    double cur = 0;
    ll speed = 1;
    for (double x : a) {
      auto time_consumed = (x - cur) / speed;
      if (time_consumed <= t) {
        cur = x;
        ++speed;
        t -= time_consumed;
  ...
  auto coord_after_rev = [&](double t) {
    double cur = l;
    ll speed = 1;
    std::for_each(std::rbegin(a), std::rend(a), [&](auto x) {
      auto time_consumed = (cur - x) / speed;
      if (time_consumed <= t) {
        cur = x;
        ++speed;
        t -= time_consumed;
  ...
  constexpr ll high = 1e16 + 1;
  auto const ans = binary_search(0, high, [&](auto t) {
    return coord_after(t * 1e-6) < coord_after_rev(t * 1e-6);
  });

  std::cout << std::fixed << std::setprecision(8) << ans * 1e-6 << endl;
}
