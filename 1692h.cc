...
auto solve(ll _t) {
  ...
  for (ll i = 0; i < n; ++i) {
    auto itr = last.find(nums[i]);
    if (itr != last.end()) {
      auto const last_idx = itr->second;
      dp[i] = std::max(dp[i], dp[last_idx] + 1 - (i - last_idx - 1));
    }
    last[nums[i]] = i;
  }

  auto const r =
      std::max_element(std::begin(dp), std::end(dp)) - std::begin(dp);
  auto const a = nums[r];
  ll l = r;
  ll sum = 0;
  for (; l >= 0; --l) {
