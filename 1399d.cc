...
auto solve(ll _t) {
  ...
  std::priority_queue<ll, std::vector<ll>, std::greater<ll>> unvisited_zero;
  std::priority_queue<ll, std::vector<ll>, std::greater<ll>> unvisited_one;
  for (ll i = 0; i < n; ++i) {
    if (str[i] == '0') {
      unvisited_zero.push(i);
    } else {
      unvisited_one.push(i);
    }
  }
  ll cur_group = 0;
  std::vector group(n, cur_group);
  for (ll i = 0; i < n; ++i) {
    if (group[i] == 0) {
      group[i] = ++cur_group;
    }
    if (str[i] == '0') {
      if (unvisited_zero.size() > 0 && unvisited_zero.top() == i) {
        unvisited_zero.pop();
      }
      if (unvisited_one.size() > 0) {
        auto top = unvisited_one.top();
        unvisited_one.pop();
        group[top] = group[i];
      }
    } else {
      if (unvisited_one.size() > 0 && unvisited_one.top() == i) {
        unvisited_one.pop();
      }
      if (unvisited_zero.size() > 0) {
        auto top = unvisited_zero.top();
        unvisited_zero.pop();
        group[top] = group[i];
      }
    }
  }
  auto max = *rng::max_element(group);
  std::cout << max << endl;
  for (auto n : group) {
    std::cout << n << ' ';
  }
