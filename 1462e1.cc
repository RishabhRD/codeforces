...
auto solve(ll _t) {
  ...
  for (auto n : nums) {
    --freq[n];
    sum += val_for(n, n) + val_for(n, n + 1) + val_for(n, n + 2) +
           val_for(n + 1, n + 1) + val_for(n + 1, n + 2) +
           val_for(n + 2, n + 2) + val_for(n, n - 1) + val_for(n, n - 2) +
           val_for(n - 1, n - 1) + val_for(n - 1, n - 2) +
           val_for(n - 2, n - 2) + val_for(n - 1, n + 1);
  }
