...
// IO {{{
namespace detail {
std::ostream &print_128(__int128 x, std::ostream &os) {
  if (x < 0) {
    os << '-';
    x = -x;
  }
...
auto solve(ll _t) {
  ...
  ll cur_added = 0;
  for (ll i = 0, pw = 1; i < 18; ++i, pw *= 10) {
    auto const dig = (n / pw) % 10;
    auto const to_add = pw * ((10 - dig) % 10);
    n += to_add;
    cur_added += to_add;
    if (get_sum(n) <= s) {
      break;
