...
auto to_val(std::string_view str) { return str != "imposter"; }

using graph_t = std::vector<std::vector<ll>>;

bool assign_colors(ll i, bool color, graph_t const &graph, ll const N,
                   std::vector<ll> &colors, ll &red, ll &blue) {
  if (colors[i] != -1) {
  ...
  for (auto n : graph[i]) {
    if (!assign_colors(n, !color, graph, N, colors, red, blue)) {
      return false;
    }
  }
...
ll solve(ll _t) {
  ...
  for (ll i = 0; i < m; ++i) {
    ...
    auto const val = to_val(read<std::string>());
    if (val) {
      graph[x].push_back(n + i);
      graph[n + i].push_back(x);
      graph[y].push_back(n + i);
      graph[n + i].push_back(y);
    } else {
      graph[x].push_back(y);
      graph[y].push_back(x);
    }
  ...
  std::vector colors(n + m, -1ll);
  for (ll i = 0; i < n; ++i) {
    if (colors[i] != -1)
      continue;
    ll red = 0;
    ll blue = 0;
    if (!assign_colors(i, false, graph, n, colors, red, blue)) {
      return -1;
    }
    sum += std::max(red, blue);
  }
