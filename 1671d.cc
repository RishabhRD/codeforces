...
auto solve(ll _t) {
  ...
  auto const min = *rng::min_element(nums);
  auto const max = *rng::max_element(nums);

  auto const min_extra = std::min({
      2 * (min - 1),
      nums.front() - 1,
      nums.back() - 1,
  });

  auto const max_extra = std::min({
      2 * std::abs(max - k),
      std::abs(nums.front() - k),
      std::abs(nums.back() - k),
  });
  ll const min_extra_contrib = min > 1 ? min_extra : 0;
  ll const max_extra_contrib = max < k ? max_extra : 0;

  std::cout << cost + min_extra_contrib + max_extra_contrib << endl;
}
