...
auto solve(ll _t) {
  ...
  std::map<ll, ll> group_num;
  {
    ll cur = 0;
    for (ll i = 0; i < n; ++i) {
      if (!group_num.count(dsu.find(i))) {
        group_num[dsu.find(i)] = cur++;
      }
    }
  }

  auto const num_groups = dsu.get_num_groups();
  std::vector freq1(num_groups, std::vector(26, 0));
  std::vector freq2(num_groups, std::vector(26, 0));
  for (ll i = 0; i < n; ++i) {
    auto const g = group_num[dsu.find(i)];
    ++freq1[g][s[i] - 'a'];
  }
