...
auto solve(ll _t) {
  ...
  if (rng::count(nums, -1ll) == n) {
    std::cout << 0 << ' ' << 0 << endl;
    return;
  }
  ll min_ele = limits<ll>::max();
  ll max_ele = limits<ll>::min();
  ll max_diff = 0;
    ...
    if (nums[i] != -1) {
      if (i + 1 < n) {
        if (nums[i + 1] != -1) {
          ...
          min_ele = std::min(min_ele, nums[i]);
          max_ele = std::max(max_ele, nums[i]);
        }
      }
      if (i - 1 >= 0) {
        if (nums[i - 1] != -1) {
  ...
  auto mid_ele = (min_ele + max_ele) / 2;
  for (ll i = 0; i < n; ++i) {
    if (nums[i] == -1)
      nums[i] = mid_ele;
  }
  ...
  std::cout << max_diff << ' ' << mid_ele << endl;
}
