...
auto solve(ll n, std::string_view str) {
  auto const cost = [&](ll k) {
    ll cur = k;
    ll sum = 0;
    for (ll i = 0; i < n; ++i) {
      if (str[i] == '*') {
  ...
  // std::cout << std::endl;
  ll const idx =
      binary_search(0, n, [&](ll i) { return cost(i - 1) > cost(i); });
  // std::cout << "idx: " << idx << ' ' << cost(idx) << std::endl;
  return cost(idx - 1);
}
