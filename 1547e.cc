...
auto solve(ll _t) {
  ...
  auto const as = read_vec<ll>(k);
  auto const ts = read_vec<ll>(k);

  std::vector<std::optional<ll>> temps(n + 2);
  for (ll i = 0; i < k; ++i) {
    temps[as[i]] = ts[i];
  }

  std::vector<ll> left(n + 1);
  std::vector<ll> right(n + 1);

  {
    ll cur_min = limits<int>::max();
    for (ll i = 1; i <= n; ++i) {
      if (temps[i].has_value()) {
        cur_min = std::min(cur_min, temps[i].value() - i);
      }
      left[i] = cur_min;
    }
  }
  ...
  for (ll i = 1; i <= n; ++i) {
    std::cout << std::min(i + left[i], right[i] - i) << ' ';
  }
  std::cout << std::endl;
}
