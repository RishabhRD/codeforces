...
ll dfs(ll i, ll prev_carry, ll prev_prev_carry, std::string_view str,
       std::vector<std::vector<std::vector<ll>>> &dp) {
  if (i == str.size()) {
    return (prev_carry == 0 && prev_prev_carry == 0) ? 1 : 0;
  }

  if (dp[i][prev_carry][prev_prev_carry] != -1)
    return dp[i][prev_carry][prev_prev_carry];

  ll const cur_dig = (str[i] - '0');
  ll const req_dig = prev_prev_carry == 0 ? cur_dig : ((cur_dig - 1 + 10) % 10);

  ll sum = 0;

  for (ll j = 0; j <= 9; ++j) {
    ll const k = req_dig - j;
    if (k >= 0 && k <= 9) {
      sum += dfs(i + 1, req_dig + prev_prev_carry >= 10, prev_carry, str, dp);
    }
  }

  for (ll j = 0; j <= 9; ++j) {
    ll const k = req_dig + 10 - j;
    if (k >= 0 && k <= 9) {
      sum +=
          dfs(i + 1, req_dig + 10 + prev_prev_carry >= 10, prev_carry, str, dp);
    }
  }

  return dp[i][prev_carry][prev_prev_carry] = sum;
}

auto solve(ll _t) {
  auto const n = read<ll>();
  std::string n_str = make_reversed(std::to_string(n));

  std::vector dp(n_str.size(), std::vector(2, std::vector(2, -1ll)));
  return dfs(0, 0, 0, n_str, dp) - 2;
}
