...
auto make_transform(auto Container, auto &&f) {
...
auto solve(ll _t) {
  ...
  auto const a = make_transform(read_vec<ll>(n), [](auto n) { return n - 1; });
  auto const b = make_transform(read_vec<ll>(n), [](auto n) { return n - 1; });
  auto const c = make_transform(read_vec<ll>(m), [](auto n) { return n - 1; });

  std::vector<std::vector<ll>> required(n);
  std::vector<std::vector<ll>> all(n);
  for (ll i = 0; i < n; ++i) {
    if (a[i] != b[i]) {
      required[b[i]].push_back(i);
    }
    all[b[i]].push_back(i);
  }

  std::vector<ll> res(m, -1ll);
  for (ll i = 0; i < m; ++i) {
    if (all[c[i]].empty()) {
      continue;
    }

    if (required[c[i]].size()) {
