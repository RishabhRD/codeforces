...
auto solve(ll _t) {
  ...
  auto cost = [&](auto x) {
    ll max = 0;
    for (ll i = 0; i < n; ++i) {
      max = std::max(max, ts[i] + std::abs(x - nums[i]));
    }
    return max;
  };

  auto const x = *rng::partition_point(
      vw::iota(0ll, max + 1), [&](auto x) { return cost(x) > cost(x + 1); });

  double factor = 1e7;

  std::cout << std::setprecision(8) << x / 1e7 << endl;
}
