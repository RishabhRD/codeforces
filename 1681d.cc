...
auto solve(ll _t) {
  ...
  while (q.size()) {
    ...
    while (sz--) {
      ...
      ll const cur_ele = top;
      while (top) {
        auto const cur_dig = top % 10;
        if (cur_dig != 1 && cur_dig != 0) {
          auto const next_ele = cur_dig * cur_ele;
          if (!visited.count(next_ele)) {
            visited.insert(next_ele);
            q.push(next_ele);
          }
        }
        top /= 10;
