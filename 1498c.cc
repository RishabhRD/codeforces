...
mii dfs(ll i, ll k, ll d, ll n, cuboid_t<ll> &dp) {
  if (i < 0 || i >= n || k == 1)
    return 1;

  if (dp.at(i).at(k).at(d) != -1)
    return dp[i][k][d];

  mii ans = 0;

  if (d == 0) {
    ans += dfs(i + 1, k, 0, n, dp);
    if (k != 1) {
      ans += dfs(i - 1, k - 1, 1, n, dp);
    }
  } else {
    ans += dfs(i - 1, k, 1, n, dp);
    if (k != 1) {
      ans += dfs(i + 1, k - 1, 0, n, dp);
    }
  }

  return dp[i][k][d] = ans.n;
}
...
auto solve(ll _t) {
  ...
  std::vector dp(n, std::vector(k + 2, std::vector(2, -1ll)));

  auto const ans = dfs(0, k, 0, n, dp).n;
  std::cout << ans << endl;
}
