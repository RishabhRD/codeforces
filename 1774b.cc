...
auto ceil_div(ll a, ll b) { return (a + b - 1) / b; }
...
auto solve(ll _t) {
  ...
  auto const a = read_vec<ll>(m);

  auto const p = ceil_div(n, k);

  if (rng::any_of(a, rd::bind_back(std::greater<>{}, p))) {
    return false;
  }

  auto const max = (n % k == 0) ? k : n % k;

  auto const eq = rng::count(a, p);
  return eq <= max;
}
