...
namespace rd {
template <typename T> struct bit_shift_left {
  constexpr auto operator()(T const &a, T const &b) const { return a << b; }
};
}; // namespace rd
...
auto solve(ll _t) {
  ...
  ll prev_max = nums[0];
  for (ll i = 1; i < n; ++i) {
    max_diff = std::max(max_diff, prev_max - nums[i]);
    prev_max = std::max(prev_max, nums[i]);
  }
  if (max_diff == 0)
    return 0ll;
  ll cur = 1;
