...
auto solve(ll _t) {
  ...
  auto a_freq_rng =
      rd::group_by(a, std::equal_to<>{}) | vw::transform([](auto &&rng) {
        return std::pair{*rng.begin(), rng::distance(rng)};
      });

  auto b_freq_rng =
      rd::group_by(b, std::equal_to<>{}) | vw::transform([](auto &&rng) {
        return std::pair{*rng.begin(), rng::distance(rng)};
      });
  ...
  for (auto [k, v] : a_freq_rng) {
    amp[k] = std::max<ll>(amp[k], v);
  }

  for (auto [k, v] : b_freq_rng) {
    bmp[k] = std::max<ll>(bmp[k], v);
  }

  ll max = 0;
  for (ll i = 1; i <= 2 * n; ++i) {
    max = std::max(amp[i] + bmp[i], max);
  }
  std::cout << max << endl;
}
