...
ll dfs(ll i, ll parent, std::vector<std::vector<ll>> const &graph,
       std::map<std::pair<ll, ll>, ll> &idx_mp) {
  ll max = 0;

  for (auto n : graph[i]) {
    if (n != parent) {
      if (idx_mp[{std::min(i, parent), std::max(i, parent)}] <
          idx_mp[{std::min(i, n), std::max(i, n)}]) {
        max = std::max(max, dfs(n, i, graph, idx_mp));
      } else {
        max = std::max(max, 1 + dfs(n, i, graph, idx_mp));
      }
    }
  }
...
auto solve(ll _t) {
  ...
  for (ll i = 0; i < n - 1; ++i) {
    ...
    idx_mp[{std::min(a, b), std::max(a, b)}] = i;
  ...
  std::cout << dfs(0, 0, graph, idx_mp) << endl;
}
