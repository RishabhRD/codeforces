...
auto solve(ll _t) {
  ...
  auto const ns = read<ll>();
  auto const nc = read<ll>();
  auto const pb = read<ll>();
  auto const ps = read<ll>();
  auto const pc = read<ll>();
  auto const r = read<ll>();

  auto const rb = rng::count(str, 'B');
  auto const rs = rng::count(str, 'S');
  auto const rc = rng::count(str, 'C');
}
