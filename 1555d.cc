...
auto solve(ll _t) {
  ...
  std::vector<std::string> all_permutation{"abc", "acb", "bac",
                                           "bca", "cab", "cba"};

  std::vector prefix(6, std::vector(n, 0ll));

  for (ll k = 0; k < 6; ++k) {
    for (ll i = 0; i < n; ++i) {
      if (all_permutation[k][i % 3] != str[i]) {
        prefix[k][i] = 1;
      }
    }
  }
  ...
  while (q--) {
    ...
    for (ll k = 0; k < 6; ++k) {
      if (l == 0) {
        min = std::min(min, prefix[k][r]);
      } else {
        min = std::min(min, prefix[k][r] - prefix[k][l - 1]);
      }
    }
    std::cout << min << endl;
  }
