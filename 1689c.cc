...
ll make_subtree_size(ll i, ll parent, std::vector<std::vector<ll>> const &tree,
                     std::vector<ll> &subtree_size) {
  ll sum = 0;
  for (auto n : tree[i]) {
    if (n != parent) {
      sum += make_subtree_size(n, i, tree, subtree_size);
    }
  }
  return subtree_size[i] = sum + 1;
}

ll max_save(ll i, ll parent, std::vector<std::vector<ll>> const &tree,
            std::vector<ll> const &subtree_size) {
  if (tree[i].size() == 1 && i != 0) {
    return 0;
  }
  if ((tree[i].size() == 2 && i != 0) || (tree[i].size() == 1 && i == 0)) {
    auto const child = tree[i][0] == parent ? tree[i][1] : tree[i][0];
    return subtree_size[child] - 1;
  }
  auto get_other = [&](ll n) -> ll {
    for (ll j = 0; j < 3; ++j) {
      if (tree[i][j] != n && tree[i][j] != parent)
        return tree[i][j];
    }
    return tree[i][2];
  };
  ...
  for (auto n : tree[i]) {
    if (n != parent) {
      auto const other = get_other(n);
      max = std::max(max, max_save(n, i, tree, subtree_size) +
                              subtree_size[other] - 1);
    }
  }
  return max;
}
...
auto solve(ll _t) {
  ...
  for (ll i = 0; i < n - 1; ++i) {
    ll const a = read<ll>() - 1;
    ll const b = read<ll>() - 1;
    tree[a].push_back(b);
    tree[b].push_back(a);
  }
  std::vector<ll> subtree_size(n);
  make_subtree_size(0, -1, tree, subtree_size);
  std::cout << max_save(0, -1, tree, subtree_size) << endl;
}
